...
class Graph(ABC):
    ...
    async def arun(self, input: Dict[str,str], 
        ...
        for round in range(num_rounds):
            ...
            if round <= 5 and skip:
                ...
                loss_f_list = []
                log_list=[]
                num_nodes = len(self.nodes)  # Use actual number of nodes
                for node_id, node in self.nodes.items():
                    ...
                    for last_node in node.spatial_successors:
                        ...
                        logits_count+=t*self.spatial_logits_1[round][in_id*num_nodes+last_id]
                        loss_t+=torch.log(1-torch.sigmoid(self.spatial_logits_1[round][in_id*num_nodes+last_id]))
                        loss_f+=torch.log(torch.sigmoid(self.spatial_logits_1[round][in_id*num_nodes+last_id]))
                    for last_node in node.spatial_predecessors:
                        last_id = list(self.nodes).index(last_node.id)
                        count+=1
                        # logits_count+=torch.sigmoid(t*self.spatial_logits_1[round][last_id*5+in_id])
                        logits_count+=t*self.spatial_logits_1[round][last_id*num_nodes+in_id]
                        loss_t+=torch.log(1-torch.sigmoid(self.spatial_logits_1[round][last_id*num_nodes+in_id]))
                        loss_f+=torch.log(torch.sigmoid(self.spatial_logits_1[round][last_id*num_nodes+in_id]))
                    # for last_node in node.temporal_predecessors:
                    #     last_id = list(self.nodes).index(last_node.id)
                    #     count+=1
                    #     # logits_count+=torch.sigmoid(t*self.temporal_logits_1[round-1][last_id*5+in_id])
                    #     logits_count+=t*self.temporal_logits_1[round-1][last_id*num_nodes+in_id]
                    #     loss_t+=torch.log(1-torch.sigmoid(self.temporal_logits_1[round-1][last_id*num_nodes+in_id]))
                    #     loss_f+=torch.log(torch.sigmoid(self.temporal_logits_1[round-1][last_id*num_nodes+in_id]))
                ...
                # selected_index = 1
                for i in range(num_nodes):  # Use actual number of nodes
                    if i==selected_index:
    ...
    def update_masks_dec(self):
        ...
        # spatial_mask_train = [param.reshape((num_nodes, num_nodes)) for param in self.spatial_masks]
        # temporal_mask_train = [param.reshape((num_nodes, num_nodes)) for param in self.temporal_masks]

        for i in range(len(spatial_matrix_train)):
            min = 100
            min_node = -1
            for j in range(num_nodes):  # Use actual number of nodes
            ...
            for k in range(num_nodes):  # Use actual number of nodes
